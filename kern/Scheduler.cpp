...
void Scheduler::newProcess(u32 function, u32 stack) {
    Memory::fill(reinterpret_cast<char*>(&g_process[g_nb_process]), 0, sizeof(g_process[g_nb_process]));
    ...
    g_process[g_nb_process].context.cs = (GDT_SEGMENT_USER_CODE*sizeof(gdt_descriptor)) | RING3;
    g_process[g_nb_process].context.ds = (GDT_SEGMENT_USER_DATA*sizeof(gdt_descriptor)) | RING3;
    g_process[g_nb_process].context.ss = (GDT_SEGMENT_USER_STACK*sizeof(gdt_descriptor)) | RING3;
    g_nb_process++;
}
...
void Scheduler::schedule(Context* context) {
    ...
    if (g_process[g_current_pid].active) {
        Memory::copy(reinterpret_cast<char*>(&(g_process[g_current_pid].context)), reinterpret_cast<char*>(context), sizeof(g_process[g_current_pid].context));
    }
