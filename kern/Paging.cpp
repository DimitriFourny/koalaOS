...
void Paging::init() {
  ...
  page_directory[0] = reinterpret_cast<u32>(page_table) |
                      PAGING_PAGE_READWRITE | PAGING_PAGE_ENABLE;
  for (unsigned int i = 0; i < PAGING_PAGE_NB_LINE; i++) {
    page_table[i] = (i * PAGING_PAGE_ENTRY_SIZE) | PAGING_PAGE_READWRITE |
                    PAGING_PAGE_ENABLE;
  }
  ...
  page_directory[index] = reinterpret_cast<u32>(page_table) | PAGING_PAGE_USER |
                          PAGING_PAGE_READWRITE | PAGING_PAGE_ENABLE;
  for (unsigned int i = 0; i < PAGING_PAGE_NB_LINE; i++) {
    page_table[i] = (GDT_USERLAND_PHYSICAL_BASE + i * PAGING_PAGE_ENTRY_SIZE) |
                    PAGING_PAGE_USER | PAGING_PAGE_READWRITE |
                    PAGING_PAGE_ENABLE;
  }
